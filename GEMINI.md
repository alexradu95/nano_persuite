# Development Guidelines for Claude/Gemini
...
# Personal Dashboard Architecture Context Document

## Project Overview

This is a personal dashboard application designed with extreme simplicity and longevity in mind. The architecture prioritizes "boring technology" that will work the same way in 10 years, similar to how Markdown files remain readable regardless of technological changes.

## Core Philosophy

- **Simplicity First**: No unnecessary abstractions or frameworks
- **Web Standards**: Use native browser capabilities whenever possible
- **Single Source of Truth**: SQLite database stores all data
- **Progressive Enhancement**: Start with HTML, enhance with minimal JavaScript
- **Separation of Concerns**: Clear distinction between hypermedia (HTML) and data (JSON) APIs

## Technology Stack

### Runtime: Bun
- **What**: All-in-one JavaScript runtime (alternative to Node.js)
- **Why**: Fast startup, built-in SQLite support, native TypeScript, no build step needed
- **Key Features Used**:
  - `bun:sqlite` - Native SQLite driver
  - `Bun.serve()` - HTTP server
  - Hot reloading with `bun --hot`
  - Built-in TypeScript support

### Frontend: HTMZ
- **What**: 166-byte HTML microframework for swapping page fragments
- **Why**: Simpler than HTMX, no special attributes, just vanilla HTML
- **How it works**:
  ```html
  <!-- Include HTMZ -->
  ...
  <!-- Usage -->
  <a href="/app/tasks/1#task-item">Load Task</a>
  <form action="/app/tasks/add#tasks-list" method="POST">
  ```
- **Key Concept**: URL hash (`#element-id`) specifies which DOM element to replace

### Database: SQLite
- **What**: File-based SQL database
- **Why**: Zero configuration, ACID compliant, perfect for personal apps
- **Location**: Single `personal.db` file in project root
- **Access**: Direct SQL queries via `bun:sqlite`, no ORM

### Styling: Tailwind CSS
- **What**: Utility-first CSS framework
- **Why**: No build step needed with CDN version, responsive by default
- **Usage**: Loaded via CDN in production, can be compiled for optimization

### Real-time Updates: Server-Sent Events (SSE)
- **What**: One-way server-to-client communication over HTTP
- **Why**: Simpler than WebSockets for server-push scenarios
- **Implementation**: `/events` endpoint streams database changes to all connected clients
...
```
/app/*  → HTML responses (for HTMZ)
/api/*  → JSON responses (for AI agents, mobile apps)
/events → SSE endpoint for real-time updates
```

### 2. No Content Negotiation

Instead of using Accept headers to serve different content types from the same endpoint, we explicitly separate HTML and JSON routes. This follows the principle that hypermedia APIs and data APIs have fundamentally different requirements.
...
-- Specific tables for each domain
transactions (amount, category, description, date)
tasks (title, completed, due_date)
journal_entries (content, mood)
health_metrics (metric_type, value, unit, recorded_at)
```

### 4. Real-time Update Flow

1. Client connects to `/events` SSE endpoint
2. When database changes occur, server notifies all SSE clients
3. Client JavaScript receives notification and triggers HTMZ reload of affected sections
4. HTMZ fetches updated HTML fragment and swaps it in the DOM

### 5. MCP (Model Context Protocol) Integration

- Discovery endpoint: `/.well-known/mcp`
- Execution endpoint: `/api/mcp/execute`
- Provides structured tool definitions for AI agents
- Same business logic serves both human (HTML) and AI (JSON) clients
...
class TransactionRepository {
  ...
  getSpendingByCategory(userId: string): SpendingSummary[]
}
```
...
async function handleHTMLRoutes(pathname: string, req: Request): Promise<Response> {
  if (pathname === '/app/finance') {
    const data = repo.getAll(userId);
    return new Response(renderFinance(data), {
...
1. **Add Database Table**
   ```typescript
   db.run(`CREATE TABLE IF NOT EXISTS new_feature (...)`)
   ```
...
3. **Add HTML Route Handler**
   ```typescript
   if (pathname === '/app/new-feature') {
     // Return HTML for HTMZ
   }
   ```

4. **Add API Route Handler**
   ```typescript
   if (pathname === '/api/new-feature') {
     // Return JSON for API
   }
   ```

5. **Create Template Function**
   ```typescript
   function renderNewFeature(data): string {
     // Return HTML string
   }
   ```
...
- Template functions instead of template engines
- Plain HTML forms instead of complex state management

### 2. HTMZ Usage
- Always include element ID in URL hash for targeted updates
- Return only the HTML fragment that needs updating
- Use forms for any action that changes data
- Use links for navigation that doesn't change data

### 3. Database Operations
- Always use parameterized queries to prevent SQL injection
- Notify SSE clients after any data modification
...
- Gracefully handle SSE disconnections
...
### Manual Testing
- Use browser developer tools to inspect HTMZ requests
...
test("TransactionRepository.create", async () => {
  const transaction = repo.create({ amount: 50, category: "food" });
  expect(transaction.id).toBeDefined();
});

// Integration tests for routes
test("POST /app/finance/add", async () => {
  const response = await fetch("/app/finance/add", {
    method: "POST",
    body: new FormData(...)
  });
...
2. Enable Bun's production mode
...
cp personal.db "backup/personal-$(date +%Y%m%d-%H%M%S).db"
```
...
```typescript
if (pathname === '/app/upload' && req.method === 'POST') {
...
db.run(`CREATE VIRTUAL TABLE search_index USING fts5(content, table_name, row_id)`);
...
1. **HTMZ not updating elements**
   - Verify the hash in the URL matches an element ID
   - Check that the response contains valid HTML
   - Ensure base target is set to "htmz"
...
### Planned Features
1. **Authentication System**: Cookie-based sessions
2. **Data Export**: CSV/JSON export functionality  
3. **Backup System**: Automated SQLite backups
4. **Search**: Full-text search across all modules
5. **Mobile App**: Progressive Web App support

### Potential Integrations
1. **Vector Database**: For semantic search in journal
2. **LLM Integration**: For intelligent insights
3. **CalDAV**: Calendar synchronization
4. **Webhooks**: For external integrations

This architecture is designed to be understood quickly by both humans and AI agents. The patterns are consistent throughout, making it easy to add new features by following the established conventions.